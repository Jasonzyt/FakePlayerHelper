...
namespace FPHelper
{
	...
	void WebSocket::connect_ws()
	{
		...
		if (this->ws)
		{
			...
			sendTextAll(LANG("gamemsg.ws.connected"));
			process();
			getVersion();
		}
		else
			...
			if (it.second == -1)
			{
				...
				if (resp.set)
				{
					...
					case FPHelper::WebSocket::PacketType::Connect:
						...
						{
							PRINT<ERROR, RED>(lpk->localize("console.failed.add.fakeplayer.format", resp.reason.c_str()));
							sendTextAll(lpk->localize("gamemsg.failed.add.fakeplayer.format", resp.reason.c_str()));
						}
						break;
					...
					case FPHelper::WebSocket::PacketType::Disconnect:
						if (resp.success)
						{
							...
							sendMessageAll(lpk->localize("gamemsg.successfully.remove.fakeplayer"));
						}
						else
						{
							PRINT<ERROR, RED>(lpk->localize("console.failed.remove.fakeplayer.format", resp.reason.c_str()));
							sendMessageAll(lpk->localize("gamemsg.failed.remove.fakeplayer.format", resp.reason.c_str()));
						}
						break;
					...
					case FPHelper::WebSocket::PacketType::GetVersion:
						PRINT(lpk->localize("fpws.version.msg.format", resp.version.c_str()));
						sendMessageAll(lpk->localize("gamemsg.ws.fp.version", resp.version.c_str()));
						break;
						...
						for (auto& it : resp.players)
						{
							if (it.state == FPState::CONNECTED)
							{
								if (!IsFakePlayer(it.name))
								{
									...
									if (pl)
									{
										fpList.push_back(new FakePlayer(pl));
										PRINT(lpk->localize("console.found.new.fakeplayer.format", it.name));
										sendMessageAll(lpk->localize("gamemsg.found.new.fakeplayer.format", it.name));
									}
								}
							}
...
#pragma region Event
	...
	{
		auto it = fpListAll.begin();
		for (; it != fpListAll.end(); it++)
		{
			if (*it == msg.name)
			{
				fpListAll.erase(it);
	...
	void WebSocket::onConnect(Message msg) 
	{
		...
		{
			waitList.push_back(make_unique<FakePlayer>(msg.name, "[Unknown]", 0));
			PRINT(lpk->localize("console.found.new.fakeplayer.format", msg.name));
			sendMessageAll(lpk->localize("gamemsg.found.new.fakeplayer.format", msg.name));
		}
	}
...
#pragma region SendWebsocketMessage
	...
	bool WebSocket::send(WSPacket pkt)
	{
		...
		if (!ws || ws->getReadyState() == easywsclient::WebSocket::CLOSED) return false;
		unique_ptr<FakePlayer> fp(pkt.target);
		switch (pkt.pt)
		{
